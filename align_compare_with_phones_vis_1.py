...
import whisperx   # якщо залишаєш вирівнювання
...
ASR_MODEL = "small.en"            # англомовна модель покращує стабільність коротких записів
BEEP_FREQ = 880                   # Hz
BEEP_DUR = 0.5                    # s
...
def levenshtein_ops(ref_tokens, hyp_tokens):
    ...
    Повертає список: (op, ref_word, hyp_word, ref_idx, hyp_idx)
      insert: ref_word='<eps>', ref_idx=None
...
def peak_normalize_wav(in_path: str, out_path: str, target_peak=0.891) -> str:
    """
    Пікова нормалізація до ~-1 dBFS (0.891) і збереження PCM_16.
    """
...
def choose_align_lang(default="en"):
    # фіксуємо 'en' для стабільності коротких англомовних фраз
    return default

def extract_words_with_times(aligned_segments):
    """
    Повертає список слів (у тій послідовності, як від aligner) з їх time-span.
    Це слова з ТЕКСТУ, який подавали в align (ref або ASR).
    """
    out = []
...
def build_token_time_map(tokens, aligned_words):
    """
    Відображення: індекс токена -> (start,end), послідовне зіставлення за нормалізованим словом.
    """
    times = []
...
def attach_ops_timing_by_side(ops, ref_tokens, hyp_tokens, aligned_words, strict_mode: bool):
    ...
    for op, r, h, i_ref, j_hyp in ops:
        ...
        rows.append({
            ...
            "ref_index": i_ref,
            "hyp_index": j_hyp,
            "word_for_timing": word_for_timing,
            "start": start,
            "end": end
        })
    return rows
...
# ---------- G2P / Phoneme fallback ----------
def g2p_fallback(word: str, lang_code: str):
    """
    Повертає список 'фонем' для слова.
    Пріоритет:
      - якщо en та доступний g2p_en → ARPAbet (без цифр)
      - якщо доступний phonemizer → IPA через espeak
      - інакше → графемний fallback (символи)
    """
    w = word.strip()
    if not w:
        return []
    if lang_code.startswith("en") and _G2P_EN is not None:
        phones = [p for p in _G2P_EN(w) if isinstance(p, str)]
        phones = [re.sub(r"\d", "", p) for p in phones if re.match(r"^[A-Z]+(\d)?$", p)]
        if phones:
            return phones
    if phonemize is not None:
        try:
            ipa = phonemize(w, language=lang_code, backend="espeak", strip=True, preserve_punctuation=False)
            phones = ipa.split()
            if phones:
                return phones
        except Exception:
            pass
    # графемний fallback
    letters = re.findall(r"[0-9A-Za-zА-Яа-яЁёІіЇїЄє’']+", w, flags=re.UNICODE)
    if letters:
        return list("".join(letters))
    return list(w)

def build_phone_rows_with_fallback(aligned_segments, word_ops_rows, align_lang: str):
    """
    Формує список фонемних рядків з таймкодами.
    Якщо у вирівнюванні є w['phones'] — беремо їх;
    інакше G2P/графемний fallback (рівномірний поділ часу слова).
    """
    ...
    phone_rows = []
    found_native_phones = False

    for seg in aligned_segments:
        for w in seg.get("words", []):
            w_norm = normalize_text(w.get("word",""))
            w_start, w_end = w.get("start", None), w.get("end", None)
            if w_start is None or w_end is None or not w_norm:
                continue
            op = idx.get((w_norm, w_start, w_end))
            if op is None:
                continue

            # 1) Спроба взяти нативні phones з aligner
            if "phones" in w and w["phones"]:
                local = []
                for ph in w["phones"]:
                    p = ph.get("phone","")
                    ps, pe = ph.get("start", None), ph.get("end", None)
                    if p and ps is not None and pe is not None:
                        local.append((p, ps, pe))
                if local:
                    found_native_phones = True
                    for p, ps, pe in local:
                        phone_rows.append({"phone": p, "start": ps, "end": pe, "op": op})
                    continue

            # 2) Fallback: G2P / графеми — рівномірно розкидаємо по інтервалу слова
            approx = g2p_fallback(w_norm, align_lang) or list(w_norm)
            n = len(approx)
    ...
    return phone_rows, found_native_phones

def plot_phones_from_rows(rows, out_png, has_native):
    rows2 = sorted(rows, key=lambda r: r["start"])
    if not rows2:
        print(f"[!] Timeline is empty, skip plot {out_png}")
        return
    y = list(range(len(rows2)))
    ...
    fig, ax = plt.subplots(figsize=(14, max(4, 0.25*len(rows2))))
    ax.barh(y, durs, left=starts, color=colors)
    ax.set_xlabel("Time (s)"); ax.set_ylabel("Phones (index)")
    title = "Phoneme timeline"
    title += " (WhisperX phones)" if has_native else " (G2P/grapheme fallback)"
    ax.set_title(title)
...
def pick_compute_type(device: str) -> str:
    # Безпечні значення для faster-whisper/ctranslate2
    return "int8" if device == "cpu" else "float16"

# === Тут вставляєш переписану функцію main() з попереднього повідомлення ===
def main(base_name="sentence_1"):
    ...
    # 3) Запис користувача (без додаткових перевірок та нормалізації)
    rec_sec = estimate_record_seconds(ref_text)
    record_user_int16(rec_sec, SR_RECORD, out_path=user_wav)

    # 4) ASR (WhisperX) — беремо segments і формуємо hyp_text з них
    ...
    # Жодних beam/VAD/temperature — простий, стабільний виклик
    asr_res = asr_model.transcribe(
        user_wav,
        language="en",
        task="transcribe",
    )

    # Нормалізовано читаємо segments з будь-якої форми (dict або list)
    if isinstance(asr_res, dict):
        segs = asr_res.get("segments", []) or []
    elif isinstance(asr_res, list):
        segs = asr_res
    else:
        segs = []

    hyp_text = " ".join(s.get("text", "").strip() for s in segs).strip()

    print(f"\n———— ASR ————")
    print("Гіпотеза (ASR):", hyp_text)
    print("Мова (задано): en")
    ...
    wer_score = wer(ref_norm, hyp_norm) if hyp_norm else 1.0
    print(f"\n📊 WER = {wer_score:.3f}")

    # 6) Forced alignment — STRICT під референс або під ASR-сегменти (використовуємо user_wav)
    ...
    align_lang = choose_align_lang("en")  # фіксовано 'en' для стабільності
    ...
    # 7) Слова з таймкодами з вирівнювання
    ...
    phone_rows, has_native = build_phone_rows_with_fallback(aligned["segments"], op_rows, align_lang)
    if phone_rows:
        plot_phones_from_rows(phone_rows, png_phones, has_native)
    else:
        print(f"[!] Timeline is empty, skip plot {png_phones}")

    # 11) Короткий звіт
    report = []
    report.append(f"Reference TXT: {ref_txt_path}")
    report.append(f"Reference WAV: {ref_wav_path}")
    report.append(f"User WAV:      {user_wav} (PCM_16)")
    report.append(f"ASR model:     {ASR_MODEL}  | device: {device} | compute_type={compute_type}")
    report.append(f"ASR language:  en (forced)")
    ...
    report.append(f"Aligned words: {len(aligned_words)}")
    report.append(f"Timed rows:    {valid_timed} / {len(op_rows)}")
    report.append(f"Phones source: {'WhisperX phones' if has_native else 'G2P/grapheme fallback'}")
